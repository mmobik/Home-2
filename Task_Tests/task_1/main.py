...
# Импорт main из Tasks/task_1.py через importlib
spec = importlib.util.spec_from_file_location("task_main", os.path.abspath(os.path.join(os.path.dirname(__file__), "../../Tasks/task_1.py")))
...
def is_small_test(input_data, threshold=300):
    """
    Проверяет, является ли тест легким (по количеству строк входа)
    """
    input_lines = input_data.count('\n') + 1 if input_data else 0
    return input_lines <= threshold
...
def run_tests_simple():
    ...
    for i, case in enumerate(test_cases, 1):
        ...
        try:
            ...
            # Проверка результата, большие тесты просто проходят (large_result)
