...
# Импорт main из Tasks/task_3.py через importlib
spec = importlib.util.spec_from_file_location("task_main", os.path.abspath(os.path.join(os.path.dirname(__file__), "../../Tasks/task_3.py")))
...
def is_small_test(input_data, threshold=100):
    """
    Проверяет, является ли тест легким (по количеству элементов N)
    """
    if not input_data:
        return True
    try:
...
def save_test_result(test_number, description, input_data, result, expected, status, time_taken, memory_used, is_small):
    ...
    with open(filename, mode, encoding='utf-8') as f:
        ...
        # Для больших тестов (отдельные файлы) выводим полные данные
        f.write(input_data + "\n\n")
        f.write("ОЖИДАЕМЫЙ РЕЗУЛЬТАТ:\n")
        # Для больших тестов выводим полный результат
...
def run_tests_simple():
    ...
    for i, case in enumerate(test_cases, 1):
        ...
        try:
            ...
            # Проверка результата (сравниваем строки построчно)
            if result is not None:
                if expected == "large_result":
                    # Для больших тестов просто проверяем, что решение выполнилось без ошибок
                    status = "✅ ПРОЙДЕН"
                    passed_tests += 1
                else:
