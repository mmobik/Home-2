...
# Импорт main из Tasks/task_4.py через importlib
spec = importlib.util.spec_from_file_location("task_main", os.path.abspath(os.path.join(os.path.dirname(__file__), "../../Tasks/task_4.py")))
...
def is_small_test(input_data, threshold=100):
    """
    Проверяет, является ли тест легким (по количеству слов N)
    """
    if not input_data:
        return True
    try:
