"""
Модуль для запуска тестов задачи 7: Привидение Ваня и плитки
...
def run_tests():
    ...
    4. Сохраняет результаты в отдельные текстовые файлы для каждого теста
    ...
    print_and_save("=" * 80)
    print_and_save("ИТОГОВЫЙ ОТЧЕТ ПО ТЕСТИРОВАНИЮ")
    print_and_save("=" * 80)
    print_and_save()
    ...
    solution_path = os.path.abspath(os.path.join(current_dir, "..", "..", "Tasks", "task_7.py"))
    ...
    for i, case in enumerate(test_cases, 1):
        ...
        # Определяем, большой ли тест (более 1000 символов в input)
        ...
        try:
            with open(test_filepath, 'w', encoding='utf-8') as f:
                ...
                if is_large_test:
                    ...
                    f.write("\nПримечание: Для больших тестов ожидаемый результат не сохраняется,\n")
                    f.write("так как он генерируется той же функцией, что и тестируется.\n")
                else:
    ...
    try:
        ...
        print(f"Результаты каждого теста сохранены в файлы: test_XX_result.txt")
    except Exception as e:
